...
// ── MAINT009-missing-default-case: Python positive fixture produces at least one finding ──

#[test]
fn python_missing_default_case_positive_fixture_has_maint009_finding() {
    let path = workspace_path("fixtures/python/missing_default_case/positive.py");
    let raw = zuit()
        .args([
            "analyze",
            path.to_str().unwrap(),
    ...
    let has_maint009 = findings
        .iter()
        .any(|f| f["rule_id"].as_str() == Some("MAINT009-missing-default-case"));

    assert!(
        has_maint009,
        "python/missing_default_case/positive.py must produce at least one \
         MAINT009-missing-default-case finding; rule_ids present: {:?}",
        findings
            .iter()
            .filter_map(|f| f["rule_id"].as_str())
            .collect::<Vec<_>>()
...
// ── MAINT010-infinite-loop-no-exit: Python positive fixture produces at least one finding ──

#[test]
fn python_infinite_loop_no_exit_positive_fixture_has_maint010_finding() {
    let path = workspace_path("fixtures/python/infinite_loop_no_exit/positive.py");
    let raw = zuit()
        .args([
            "analyze",
            path.to_str().unwrap(),
    ...
    let has_maint010 = findings
        .iter()
        .any(|f| f["rule_id"].as_str() == Some("MAINT010-infinite-loop-no-exit"));

    assert!(
        has_maint010,
        "python/infinite_loop_no_exit/positive.py must produce at least one \
         MAINT010-infinite-loop-no-exit finding; rule_ids present: {:?}",
        findings
            .iter()
            .filter_map(|f| f["rule_id"].as_str())
            .collect::<Vec<_>>()
...
// ── SEC012-hardcoded-security-constant: Python positive fixture produces at least one finding ──

#[test]
fn python_hardcoded_security_constant_positive_fixture_has_sec012_finding() {
    let path = workspace_path("fixtures/python/hardcoded_security_constant/positive.py");
    let raw = zuit()
        .args([
            "analyze",
            path.to_str().unwrap(),
    ...
    let has_sec012 = findings
        .iter()
        .any(|f| f["rule_id"].as_str() == Some("SEC012-hardcoded-security-constant"));

    assert!(
        has_sec012,
        "python/hardcoded_security_constant/positive.py must produce at least one \
         SEC012-hardcoded-security-constant finding; rule_ids present: {:?}",
        findings
            .iter()
            .filter_map(|f| f["rule_id"].as_str())
            .collect::<Vec<_>>()
...
// ── SEC014-redos-regex: Python positive fixture produces at least one finding ──

#[test]
fn python_redos_regex_positive_fixture_has_sec014_finding() {
    let path = workspace_path("fixtures/python/redos_regex/positive.py");
    let raw = zuit()
        .args([
            "analyze",
            path.to_str().unwrap(),
    ...
    let has_sec014 = findings
        .iter()
        .any(|f| f["rule_id"].as_str() == Some("SEC014-redos-regex"));

    assert!(
        has_sec014,
        "python/redos_regex/positive.py must produce at least one \
         SEC014-redos-regex finding; rule_ids present: {:?}",
        findings
            .iter()
            .filter_map(|f| f["rule_id"].as_str())
            .collect::<Vec<_>>()
...
// ── SEC015-log-injection: Python positive fixture produces at least one finding ──

#[test]
fn python_log_injection_positive_fixture_has_sec015_finding() {
    let path = workspace_path("fixtures/python/log_injection/positive.py");
    let raw = zuit()
        .args([
            "analyze",
            path.to_str().unwrap(),
    ...
    let has_sec015 = findings
        .iter()
        .any(|f| f["rule_id"].as_str() == Some("SEC015-log-injection"));

    assert!(
        has_sec015,
        "python/log_injection/positive.py must produce at least one \
         SEC015-log-injection finding; rule_ids present: {:?}",
        findings
            .iter()
            .filter_map(|f| f["rule_id"].as_str())
            .collect::<Vec<_>>()
...
// ── MAINT012-dead-store: Python positive fixture produces at least one finding ──

#[test]
fn python_dead_store_positive_fixture_has_maint012_finding() {
    let path = workspace_path("fixtures/python/dead_store/positive.py");
    let raw = zuit()
        .args([
            "analyze",
            path.to_str().unwrap(),
    ...
    let has_maint012 = findings
        .iter()
        .any(|f| f["rule_id"].as_str() == Some("MAINT012-dead-store"));

    assert!(
        has_maint012,
        "python/dead_store/positive.py must produce at least one \
         MAINT012-dead-store finding; rule_ids present: {:?}",
        findings
            .iter()
            .filter_map(|f| f["rule_id"].as_str())
            .collect::<Vec<_>>()
