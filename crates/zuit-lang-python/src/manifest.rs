//! `PythonManifest` — per-project cache of parsed `pyproject.toml` and related
//! project-root metadata.
//!
//! All PKG (and future HEALTH/CHAIN) analyzers share a single `Arc<PythonManifest>`
//! per project root so that `pyproject.toml` is read and parsed at most once per
//! engine run.
//!
//! # Thread safety
//!
//! The global cache is a `OnceLock<Mutex<HashMap<PathBuf, Arc<PythonManifest>>>>`.
...
/// Project-level metadata for a Python project, derived from `pyproject.toml`
...
pub(crate) struct PythonManifest {
    /// Canonical project root path (used as the cache key).
    #[allow(dead_code)]
    pub root: PathBuf,

    /// Parsed `pyproject.toml` document, or `None` if the file does not exist.
    pub pyproject: Option<toml_edit::DocumentMut>,

    /// Absolute path to `pyproject.toml`, or `None` if absent.
    pub pyproject_path: Option<PathBuf>,

    /// Parse error from `pyproject.toml`, if the file exists but is not valid
    /// TOML.  The tuple is `(message, (line, col))` — best-effort from
    /// `toml_edit::TomlError`.
    pub pyproject_parse_error: Option<(String, (u32, u32))>,
    ...
    ///
    /// Populated lazily on the first call to [`PythonManifest::git_log`].
...
static CACHE: OnceLock<Mutex<HashMap<PathBuf, Arc<PythonManifest>>>> = OnceLock::new();

fn cache() -> &'static Mutex<HashMap<PathBuf, Arc<PythonManifest>>> {
...
/// Returns the `PythonManifest` for `project`, reading and parsing
/// `pyproject.toml` on the first call for each distinct project root.
///
/// Subsequent calls for the same canonical root return the cached `Arc`
/// without re-reading the file.
pub(crate) fn manifest_for(project: &Project) -> Arc<PythonManifest> {
...
fn build_manifest(root: PathBuf) -> PythonManifest {
    let pyproject_path = {
        let p = root.join("pyproject.toml");
        if p.exists() { Some(p) } else { None }
    };

    let (pyproject, pyproject_parse_error) = match &pyproject_path {
        None => (None, None),
        Some(path) => match std::fs::read_to_string(path) {
            Err(e) => {
                let msg = format!("cannot read pyproject.toml: {e}");
                (None, Some((msg, (1, 1))))
            }
