...
/// `OnceLock` on [`crate::manifest::RustManifest`].
#[derive(Debug, Clone)]
...
#[cfg(test)]
impl Commit {
    /// Constructs a [`Commit`] for unit tests without invoking `git`.
...
#[cfg(test)]
impl Tag {
    /// Constructs a [`Tag`] for unit tests without invoking `git`.
...
fn run_git(cwd: &Path, args: &[&str]) -> io::Result<String> {
    use zuit_core::external::Outcome;
    use zuit_core::external::run_with_limits;
...
mod tests {
    ...
    #[test]
    fn parse_log_output_skips_malformed_lines() {
    ...
    #[test]
    fn parse_tag_output_skips_malformed_lines() {
